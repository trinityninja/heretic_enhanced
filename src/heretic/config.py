...
class Settings(BaseSettings):
    ...
    audit: bool = Field(
        default=False,
        description="Enable the red-team audit after abliteration.",
    )

    audit_model: str | None = Field(
        default=None,
        description="If set, run the red-team audit on this model instead of abliterating.",
    )

    audit_output_dir: str = Field(
        default="audit_reports",
        description="Directory to save red-team audit reports.",
        exclude=True,
    )

    audit_builtin_prompts: bool = Field(
        default=True,
        description="Use the built-in red-team prompt set for auditing.",
    )

    audit_prompts_file: str | None = Field(
        default=None,
        description="Path to a JSONL file with custom audit prompts (category, technique, text).",
        exclude=True,
    )

    audit_categories: list[str] | None = Field(
        default=None,
        description="If set, only run audit prompts in these categories.",
    )

    audit_max_prompts: int | None = Field(
        default=None,
        description="Maximum number of audit prompts to run (useful for quick checks).",
    )
